...
export default function Home() {
  ...
  return (
    <div className={styles.page}>
      ...
      {/* AI KNOWLEDGE STUDIO & MCQ GENERATOR DEMO */}
      <section className={styles.studioDemoSection}>
        <div className="container">
          <div className={styles.studioBox}>
            <div className={styles.studioHeader}>
              <span className="badge badge-primary">AI Knowledge Studio Preview</span>
              <h2>Turn Learning Material Into Interactive Learning Experiences</h2>
              <p>Upload official manuals or datasets, and let StatPath AI create flashcards, summaries, and MCQs instantly.</p>
              <div style={{ margin: '16px 0', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--gray-200)' }}>
                <img src="/statpath_ai_studio.png" alt="StatPath AI Knowledge Studio Banner" style={{ width: '100%', maxHeight: '240px', objectFit: 'cover' }} />
              </div>
            </div>
            ...
            <div className={styles.demoUi}>
              <div className={styles.demoSidebar}>
                <div className={styles.uploadBox}>
                  📄 <strong>Sampling_Methodology_2024.pdf</strong>
                  <span className={styles.fileSize}>2.4 MB • Uploaded</span>
                </div>
                <div className={styles.formMiniGroup}>
                ...
                <div className={styles.formMiniGroup}>
                  ...
                  <select className="form-select" defaultValue="intermediate">
                    <option value="easy">Basic</option>
                    <option value="intermediate">Intermediate</option>
                    <option value="advanced">Advanced</option>
                  </select>
                </div>
                <button className="btn btn-karnataka btn-sm" style={{ width: '100%' }}>
                  ⚡ Generate Assessment
                </button>
              </div>

              {/* Sample Question Card */}
              <div className={styles.demoQuestionCard}>
                <div className={styles.qHeader}>
                  <span className={styles.qTopic}>Topic: Stratified Random Sampling</span>
                  <span className={styles.qDiff}>Difficulty: Intermediate</span>
                </div>
                <h4 className={styles.qText}>
                  In conducting a multi-stage agricultural sample survey across districts in Karnataka, why is stratification by agro-climatic zone preferred over simple random sampling?
                </h4>

                <div className={styles.optionsList}>
                  {[
                    'A. It eliminates the need for calculating standard errors in the final yield estimation.',
                    'B. It reduces sampling variance by ensuring sub-groups with similar agricultural characteristics are represented proportionally.',
                    'C. It requires zero field investigators and relies entirely on satellite remote sensing.',
                    'D. It guarantees that smaller districts are completely excluded from the sample frame.'
                  ].map((opt, idx) => (
                    <button 
                      key={idx}
                      className={`${styles.optBtn} ${mcqSubmitted === idx ? (idx === 1 ? styles.optCorrect : styles.optWrong) : ''}`}
                      onClick={() => setMcqSubmitted(idx)}
                    >
                      {opt}
                    </button>
                  ))}
                ...
                {mcqSubmitted !== null && (
                  <div className={styles.explanationBox}>
                    ...
                    <p>
                      Stratification by agro-climatic zones groups homogeneous agricultural regions, thereby reducing within-stratum variance and yielding more reliable crop production estimates.
                    </p>
                    <span className={styles.sourceRef}>Source: Manual on Agricultural Statistics, Directorate of Economics & Statistics (Page 42).</span>
                  </div>
                )}
              </div>
            </div>
